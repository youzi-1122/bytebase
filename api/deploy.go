...
func ValidateAndGetDeploymentSchedule(payload string) (*DeploymentSchedule, error) {
	...
	for _, d := range schedule.Deployments {
		if d.Name == "" {
			return nil, common2.Errorf(common2.Invalid, fmt.Errorf("Deployment name must not be empty"))
		}
		hasEnv := false
		for _, e := range d.Spec.Selector.MatchExpressions {
			...
			case InOperatorType:
				if len(e.Values) == 0 {
					return nil, common2.Errorf(common2.Invalid, fmt.Errorf("expression key %q with %q operator should have at least one value", e.Key, e.Operator))
				}
			case ExistsOperatorType:
				if len(e.Values) > 0 {
					return nil, common2.Errorf(common2.Invalid, fmt.Errorf("expression key %q with %q operator shouldn't have values", e.Key, e.Operator))
				}
			default:
				return nil, common2.Errorf(common2.Invalid, fmt.Errorf("expression key %q has invalid operator %q", e.Key, e.Operator))
			}
			if e.Key == EnvironmentKeyName {
				hasEnv = true
				if e.Operator != InOperatorType || len(e.Values) != 1 {
					return nil, common2.Errorf(common2.Invalid, fmt.Errorf("label %q should must use operator %q with exactly one value", EnvironmentKeyName, InOperatorType))
				}
			}
		}
		if !hasEnv {
			return nil, common2.Errorf(common2.Invalid, fmt.Errorf("deployment should contain %q label", EnvironmentKeyName))
		}
	}
	return schedule, nil
}
